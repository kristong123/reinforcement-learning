...
@test("q1", points=25, prereqs=[], part="p1")
...
@test("q3", points=20, prereqs=[], part="p2")
def check_q_update(tracker):
    partial_credit_per_disk = 3
    for n_disks in [2, 3, 4]:
...
def check_custom_epsilon(tracker):
    ...
    assert len(set(epsilons)) >= 10, (
        "Your custom_epsilon needs to return at least 10 different values on "
